...
- **`support/`** — `env-config.ts` (`convict`-based schema, the single source of truth for config), `ssm-config.ts` (pulls SSM parameters into `process.env` before config is built), `get-configuration.ts` (builds the `oidc-provider` `Configuration` object, including cached client lookups, resource-indicator validation, and deriving the JWKS signing keystore from the SSM-stored private key at cold start), `dynamoose-table-options.ts` (disables auto table creation/waiting outside local dev).
